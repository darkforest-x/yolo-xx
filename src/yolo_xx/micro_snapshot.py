"""Materialize immutable pre-holdout prefixes for local 1m/2m/3m/5m OHLCV.
...
from .source_manifest import HOLDOUT_START, sha256_file, utc_iso
...
def create_snapshot(
    *,
    ...
    if normalized not in {"1m", "2m", "3m", "5m"}:
        raise ValueError("micro snapshot timeframe must be 1m, 2m, 3m, or 5m")
    ...
    try:
        for source in sources:
            ...
            entry = _copy_prefix(
                source, destination, timeframe=normalized, cutoff=HOLDOUT_START
            )
            if entry is None:
...
def make_plan(
    *, cache_dir: Path, out_dir: Path, timeframe: str, min_preholdout_rows: int
) -> dict[str, object]:
