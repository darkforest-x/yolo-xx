...
3. **正负样本必须在「除标签之外的一切维度」上同分布**——渲染管线、采集批次、时间段、来源。
   做不到就**按 source 分层各报一次**；子集全线塌陷 = 来源混杂，各子集一致 = 真信号。
   （原表述只写「同一条渲染管线」，源于 v15；2026-08-06 泛化：渲染已统一仍因批次差异，
   把真实的 0.57–0.65 抬成合池 0.8067，三来源正类率 85%/84%/29%。分组 CV 防不了这个。）
